...
import { createNpc } from '../server/index'
...
export function CreateNpcForm({ campaignId, onSuccess }: CreateNpcFormProps) {
  ...
  const form = useForm({
    defaultValues: { name: '', portraitUrl: '' },
    validators: { onChange: staticFieldSchema, onBlur: staticFieldSchema },
  ...
  return (
    ...
    >
      ...
      >
        {form.state.isSubmitting ? 'Creating…' : 'Create NPC'}
      </Button>
    </form>
  )
}
